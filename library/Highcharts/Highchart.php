...
class Highcharts_Highchart implements ArrayAccess
{
    //The chart type.
    //A regullar higchart
    const HIGHCHART = 0;
    ...
    {
        include_once APPLICATION_PATH."/../library/Highcharts/conf.php";
    ...
    {
        $this->_options[$offset] = new Highcharts_HighchartOption($value);
    }
    ...
    public function offsetGet($offset)
    {
        if (!isset($this->_options[$offset])) {
            $this->_options[$offset] = new Highcharts_HighchartOption();
        }
    ...
    private static function _replaceJsExpr($data, &$jsExpressions)
    {
        ...
        if (is_object($data) &&
            !$data instanceof Highcharts_HighchartJsExpr) {
            $data = $data->getValue();
        }

        if ($data instanceof Highcharts_HighchartJsExpr) {
