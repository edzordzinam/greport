<?php
class PDFTable_Pdf_Page extends Zend_Pdf_Page {
	...
	public function setDefaultStyle(Zend_Pdf_Style $style){
	...
	public function getWidth($intContentArea=false){
		...
		if($intContentArea){
			$width-=$this->_margin[PDFTable_Pdf::LEFT];
			$width-=$this->_margin[PDFTable_Pdf::RIGHT];
		}
	 ...
	 * Get a Page margin
	 *
	 * @param PDFTable_Pdf::Position $position
	 * @return int margin
	 */
	 ...
	 * @param int $posY
	 */
	public function addTable(PDFTable_Pdf_Table $table,$posX,$posY,$inContentArea=true){
	...
	public function drawLine($x1,$y1,$x2,$y2,$inContentArea=true){
		//move origin
		if($inContentArea){
			$y1 = $this->getHeight()- $y1  - $this->getMargin(PDFTable_Pdf::TOP);
			$y2 = $this->getHeight()- $y2  - $this->getMargin(PDFTable_Pdf::TOP);
			$x1 = $x1  + $this->getMargin(PDFTable_Pdf::LEFT);
			$x2 = $x2  + $this->getMargin(PDFTable_Pdf::LEFT);
		}
	...
	public function drawImage(Zend_Pdf_Resource_Image $image,$x1,$y1,$width,$height,$inContentArea=true){
		if($inContentArea){
			$y1 = $this->getHeight()- $y1  - $this->getMargin(PDFTable_Pdf::TOP)-$height;
			$x1 = $x1  + $this->getMargin(PDFTable_Pdf::LEFT);
