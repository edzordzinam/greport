...
class PDFTable_Pdf_Table_Cell {
	 ...
	 * Cell borders
	 *
	 * @var array (PDFTable_Pdf position=>array('color','width','dashing_pattern'))
	 */
	private $_border;
	 ...
	 * Set Cell Borders
	 *
	 * @param array(array(PDFTable_Pdf position, Zend_Pdf_Styles style))
	 */
	 ...
	 *
	 * @param string $text
	 * @param PDFTable_Pdf $align Horizontal Alignment
	 * @param PDFTable_Pdf $valign Vertical Alignment
	 */
	...
	public function setImage($filename,$align=null,$valign=null,$scale=1){
		...
		if($scale>1)
			throw new Zend_Exception("Scale must be between (0,1]","PDFTable_Pdf_Table_Cell::addImage()");
	 ...
	 */
	public function preRender(PDFTable_Pdf_Page $page,$posX,$posY,$inContentArea=true) {
		if(!$this->_width){
			//no width given, get max width of page
			if($inContentArea){
				$width=$page->getWidth()-$posX-($page->getMargin(PDFTable_Pdf::LEFT)+$page->getMargin(PDFTable_Pdf::RIGHT));
			}
			else{
		...
		//calc max cell width
		$maxWidth=$width-($this->_padding[PDFTable_Pdf::LEFT]+$this->_padding[PDFTable_Pdf::RIGHT])-(+$this->_getBorderLineWidth(PDFTable_Pdf::LEFT)+$this->_getBorderLineWidth(PDFTable_Pdf::RIGHT));
		...
		if($this->_text){	
			...
			if(!$this->_width){
				//add padding
				$this->_recommendedWidth = $text_props['text_width']+($this->_padding[PDFTable_Pdf::LEFT]+$this->_padding[PDFTable_Pdf::RIGHT])+$this->_getBorderLineWidth(PDFTable_Pdf::LEFT)+$this->_getBorderLineWidth(PDFTable_Pdf::RIGHT);
			}
			else{
			...
			if(!$this->_height){
				...
				$this->_recommendedHeight = $height+($this->_padding[PDFTable_Pdf::TOP]+$this->_padding[PDFTable_Pdf::BOTTOM]);
			}
		...
		elseif($this->_image){
			...
			if(!$this->_width)
				$this->_recommendedWidth = $this->_image['scale']*$image->getPixelWidth ()+($this->_padding[PDFTable_Pdf::LEFT]+$this->_padding[PDFTable_Pdf::RIGHT])+$this->_getBorderLineWidth(PDFTable_Pdf::LEFT)+$this->_getBorderLineWidth(PDFTable_Pdf::RIGHT);
			if(!$this->_height)
				$this->_recommendedHeight = $this->_image['scale']*$image->getPixelHeight ()+($this->_padding[PDFTable_Pdf::TOP]+$this->_padding[PDFTable_Pdf::BOTTOM]);
	 ...
	 * @param int $posY
	 */
	public function render(PDFTable_Pdf_Page $page, $posX, $posY) {
	...
	private function _renderText(PDFTable_Pdf_Page $page, $posX, $posY){
		...
		if(count($this->_text['lines'])>1){
			...
			*/
			...
			$this->_vAlign=PDFTable_Pdf::TOP; //@@TODO ONLY TOP ALIGNEMENT IS VALID AT THIS MOMENT
			foreach ($this->_text['lines'] as $line){
				$page->drawText($line,$this->_getTextPosX($posX), $this->_getTextPosY($page,$y_inc));
				$y_inc+=$line_height;
			}
	...
	private function _renderImage(PDFTable_Pdf_Page $page, $posX, $posY){
	...
	private function _renderBorder(PDFTable_Pdf_Page $page, $posX, $posY) {		
		if(!$this->_border) return;
		
		foreach ($this->_border as $key=>$style){
			$page->setStyle($style);			
			switch ($key) {
				case PDFTable_Pdf::TOP:
					$page->drawLine(
									$posX, $posY-$this->_getBorderLineWidth(PDFTable_Pdf::TOP)/2,
									$posX+$this->_width, $posY-$this->_getBorderLineWidth(PDFTable_Pdf::TOP)/2,
									true
									);
					break;
				case PDFTable_Pdf::BOTTOM:
					$page->drawLine(
									$posX, $posY + $this->_height+$this->_getBorderLineWidth(PDFTable_Pdf::BOTTOM)/2, 
									$posX+$this->_width, $posY + $this->_height+$this->_getBorderLineWidth(PDFTable_Pdf::BOTTOM)/2,
									true
									);
					break;
				case PDFTable_Pdf::RIGHT:
	...
	private function  _renderBackground(PDFTable_Pdf_Page $page, $posX, $posY){
	...
	private function _getTextPosX($posX){
		/* Zend_Debug::dump($this->_text['text']);
		Zend_Debug::dump("cell width :" .$this->_width);
		Zend_Debug::dump("text width :" .$this->_text['width']);
		Zend_Debug::dump("position :" .$posX); */
		$x=0;
		switch ($this->_align){
			case PDFTable_Pdf::RIGHT:
				$x=$posX+$this->_width-$this->_text['width']-$this->_padding[PDFTable_Pdf::RIGHT]-$this->_getBorderLineWidth(PDFTable_Pdf::RIGHT)/2;
				break;
		 	case PDFTable_Pdf::CENTER:
				$x=$posX+($this->_width-$this->_text['width'])/2;
				//Zend_Debug::dump("center ================= :" .$x);
				break;  
			default: //LEFT
				$x=$posX+$this->_padding[PDFTable_Pdf::LEFT]+$this->_getBorderLineWidth(PDFTable_Pdf::LEFT)/2;
				break;
		}
		return $x;
	}
	 ...
	 * @return int
	 */
	private function _getTextPosY(PDFTable_Pdf_Page $page,$posY){
		$y=0;
		$line_height=$page->getFontHeight()+$this->_textLineSpacing;
		
		switch ($this->_vAlign){
			case PDFTable_Pdf::BOTTOM:
				$y=$posY+$this->_height-$this->_padding[PDFTable_Pdf::BOTTOM];
				break;
			case PDFTable_Pdf::MIDDLE:
				$y=$posY+$this->_height/2+$line_height/2;
				break;
			default: //TOP
				$y=$posY+$line_height+$this->_padding[PDFTable_Pdf::TOP];
				break;
		}
		return $y;
	}
	...
	private function _getImagePosX($posX){
		...
		switch ($this->_align){
			case PDFTable_Pdf::RIGHT:
				$x=$posX+$this->_width-$this->_image['width']-$this->_padding[PDFTable_Pdf::RIGHT];
				break;
			case PDFTable_Pdf::CENTER:
				$x=$posX+$this->_width/2-$this->_image['width']/2;
				break;
			default: //LEFT
				$x=$posX+$this->_padding[PDFTable_Pdf::LEFT];
				break;
		}
		return $x;
	}
	...
	private function _getImagePosY($posY){
		...
		switch ($this->_vAlign){
			case PDFTable_Pdf::BOTTOM:
				$y=$posY+$this->_height-$this->_image['height']-$this->_padding[PDFTable_Pdf::BOTTOM];
				break;
			case PDFTable_Pdf::MIDDLE:
				$y=$posY+($this->_height-$this->_image['height'])/2;
				break;
			default: //TOP
				$y=$posY+$this->_padding[PDFTable_Pdf::TOP];
				break;
		}
		return $y;
	}
