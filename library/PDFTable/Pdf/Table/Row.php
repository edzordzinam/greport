...
class PDFTable_Pdf_Table_Row {
	 ...
	 * Get Row Columns
	 *
	 * @return array(PDFTable_Pdf_Table_Column)
	 */
	 ...
	 * Set Row Columns
	 *
	 * @param array(PDFTable_Pdf_Table_Column,...) $columns
	 */
	 ...
	 *
	 * @param PDFTable_Pdf_Table_Column $col
	 * @param int index of new Coumn $index
	 */
	public function insertColumn(PDFTable_Pdf_Table_Column $col,$index){
		//after calling insert increase column count...
		
		$index += $this->_insertCount;
		$this->_insertCount += 1;
	...
	public function __construct(){
		//set default font
		$this->_font=Zend_Pdf_Font::fontWithName ( ZEND_Pdf_Font::FONT_TIMES_ROMAN);
	}
	...
	public function preRender(PDFTable_Pdf_Page $page,$posX,$posY,$inContentArea=true) {
		...
		if($this->_width){
			...
			if($inContentArea){
				$max_row_width=$page->getWidth()-$page->getMargin(PDFTable_Pdf::LEFT)-$page->getMargin(PDFTable_Pdf::RIGHT);
			}
			else{
